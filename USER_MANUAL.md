...
5. 点击“开始流播”启动内嵌播放器；HLS 会先经过本地代理，直链、DASH、Smooth 或 RTSP 会按媒体类型交给 libVLC。
...
BBDown 的网页登录方式会把登录接口返回的 Cookie 查询字段保存到应用目录下的
`BBDown.data`，属于明文保存。本项目桌面端使用配置文件保存但限制文件权限，Android
则使用 Android Keystore 加密保存 Cookie；卸载 Android 应用或清除其安全存储后，登录状态
可能丢失。Cookie 失效时重新使用二维码登录即可，不需要手工提取 Cookie。
...
为降低高频请求触发 B 站限流或 IP 风险，程序对识别到的 B 站页面、接口和媒体地址使用共享请求节流，默认相邻请求至少间隔约 `0.4` 秒。桌面端 B 站 HLS 分片最多使用 `2` 个工作线程；Android 即使设置更高线程数，B 站分片也最多使用 `2` 个并发请求。

收到 HTTP `429` 时，程序会立即停止当前请求链路，不会继续重试或快速切换地址。`401`、`403`、`404` 和 `410` 等明确失败也不会被重复轰炸；临时网络错误才使用逐步增加的退避等待。看到“请求过于频繁”后，请停止重复探测、刷新或重新粘贴同一链接，等待一段时间再进行一次操作。

线程数和重试次数不是越高越好，不要为了提速手动提高 B 站并发或反复启动多个实例。BBDown 的 `BBDown.data` 可以保留 Cookie，但其请求策略不提供本项目的统一节流保护；本项目优先保守请求频率和停止条件。任何客户端都不能保证完全避免平台限流或封禁，请遵守 B 站服务规则并只下载有权处理的内容。
