...
这是一个覆盖 Linux、Windows 和 Android 的媒体下载与流播工具，支持可选的广告过滤、本地流播和多种输出格式。

桌面端提供 CLI、图形界面和终端界面；Android 端提供流播、下载和设置入口，包名为 `com.dai2010.m3u8down`，最低支持 Android 10。

## 功能

- 支持常见媒体播放列表和多种媒体地址，并自动选择合适的播放内容。
- 用户停止输入 5 秒后自动探测媒体类型，也可以点击“立即探测”或在流播地址框按回车立即探测；识别为 HLS 后才开放 m3u8 全文预览，MP4、M4S、MP3、M4A、TS 等直链不会进入列表预览。
- 支持无扩展名媒体地址，并自动识别 HLS/m3u8、DASH/mpd、Smooth Streaming、RTSP 和常见直链音视频。
- 根据媒体类型提供相应的下载和流播体验，支持多种常见输出格式。
- 按关键词或正则过滤广告片段。
- 支持并发下载、中断后继续处理和任务进度显示。
- 支持输出 MP4 等常见媒体文件。
- 桌面 GUI 提供播放、停止、进度、倍速和全屏控制；本地流播支持可选的广告过滤。
