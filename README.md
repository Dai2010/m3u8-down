...
> B 站兼容功能已全平台下线。此前版本中的 B 站兼容功能已确认不可用，后续版本不再继续开发、修复或恢复该功能。
...
- `m3u8-downloader-5.0.3-windows-x64.exe`
- `m3u8-downloader-5.0.3-windows-x64.msi`
...
```bash
packaging/linux/build_deb.sh 5.0.3
```

发布 Windows、Android 和 Linux 资产时，可以在 GitHub Actions 中手动运行 `Release` workflow，并填写 tag，例如 `v5.0.3`。Android APK 会使用升级签名密钥签名并校验证书指纹。
...
- `m3u8-downloader-android-arm64-v8a.apk`
- `m3u8-downloader_5.0.3_amd64.deb`
- `m3u8-downloader-5.0.3-windows-x64.exe`
- `m3u8-downloader-5.0.3-windows-x64.msi`
